...
angular.module('accessimapEditeurDerApp')
    .service('LocalmapService', ['$rootScope', '$http', '$q', 'usSpinnerService', 'settings', 'initSvg', 'shareSvg', 'svgicon',
            function($rootScope, $http, $q, usSpinnerService, settings, initSvg, shareSvg, svgicon) {
         ...
         */
        var addToLegend = function(query, style, position, colorChosen, checkboxModel) {
         ...
         * 
         */
        var storeMapAndLegend = function() {
            
            var deferred = $q.defer();

            var svg    = d3.select(_mapSelector).select('svg').node(),
         ...
         * @param  {[type]} id id of the feature
         */
        var removeFeature = function(id) {
            ...
            var index = _geojson.indexOf(result[0]);
         ...
         * @param  {[type]} id id of the feature
         */
        var updateFeature = function(id, style) {
            var result = _geojson.filter(function(obj) {
                return obj.id === id;
            });
            var objectId = _geojson.indexOf(result[0]);
         ...
         * @param  {[type]} queryChosen   [description]
         * @param  {[type]} styleChosen   [description]
         * @param  {[type]} styleChoices  [description]
         * @param  {[type]} colorChosen   [description]
         * @param  {[type]} checkboxModel [description]
         */
        var simplifyFeatures = function(feature, queryChosen, styleChosen, styleChoices, colorChosen, checkboxModel, rotationAngle) {
            ...
            geojsonToSvg(data, feature.simplification / 100000, feature.id, null, queryChosen, styleChosen, styleChoices, colorChosen, checkboxModel, rotationAngle);
            //}
        }
         ...
         * @param  {[type]} feature       [description]
         * @param  {[type]} optionalClass [description]
         */
        var drawFeature = function(data, feature, optionalClass, styleChosen, colorChosen, checkboxModel, rotationAngle) {
            var featureGroup;
         ...
         */
        var rotateFeature = function(feature) {
         ...
         * @param  {[type]} optionalClass [description]
         */
        var geojsonToSvg = function(data, simplification, id, poi, queryChosen, styleChosen, styleChoices, colorChosen, checkboxModel, rotationAngle) {
            if (data) {
         ...
         * @param  {[type]} colorChosen   [description]
         * @param  {[type]} checkboxModel [description]
         * @param  {[type]} rotationAngle [description]
         */
        var downloadPoi = function(queryChosen, styleChosen, styleChoices, colorChosen, checkboxModel, rotationAngle) {
            $(_mapSelector).css('cursor', 'crosshair');
            d3.select('svg')
                .on('click', function() {
                    var coordinates = d3.mouse(this);
                    var point = _projection.invert(coordinates);
         ...
         * @param  {[type]} rotationAngle [description]
         */
        var downloadData = function(point, queryChosen, styleChosen, styleChoices, colorChosen, checkboxModel, rotationAngle) {
            ...
            if (point) {
                ...
                var boundsSE = _projection.invert([_width, _height]);
                mapS = boundsSE[1];
                mapW = boundsNW[0];
                mapN = boundsNW[1];
                mapE = boundsSE[0];
                ...
                .success(function successDownload(data) {
                    ...
                    if (queryChosen.id === 'poi') {
                        ...
                        if (osmGeojson.features[0]) {
                            geojsonToSvg(osmGeojson, undefined, 'node_' + osmGeojson.features[0].properties.id, true, queryChosen, styleChosen, styleChoices, colorChosen, checkboxModel, rotationAngle);
                        }
                    } else {
                        geojsonToSvg(osmGeojson, undefined, undefined, null, queryChosen, styleChosen, styleChoices, colorChosen, checkboxModel, rotationAngle);
                    }
         ...
         */
        var zoomOnPlace = function(address, styleChosen, colorChosen, checkboxModel, rotationAngle) {
            var start = address.start !== '' && address.start;
            var stop = address.stop !== '' && address.stop;
            if (start && stop) {
                    ...
                    if (dataStart[0]) {
                        ...
                        $http.get(urlStop).success(function(dataStop) {
                            if (dataStop[0]) {
                                ...
                                var s = _zoom.scale() / Math.max(Math.abs(locationStart[0] - locationStop[0]) / _width, Math.abs(locationStart[1] - locationStop[1]) / _height) / 1.2;
                                p = d3.geo.mercator()
                                        .scale(s / 2 / Math.PI)
                                        .translate([_width / 2, _height / 2]);
                                var coordinates = [(lonStart + lonStop) / 2, (latStart + latStop) / 2];
                                var location = p(coordinates);
                                var translateX = _width - location[0],
                ...
                $http.get(url).
                    success(function(data) {
                        if (data[0]) {
                            ...
                            var t = [_width - location[0], _height - location[1]];
         ...
         */
        var init = function(mapFormat, legendFormat) {
